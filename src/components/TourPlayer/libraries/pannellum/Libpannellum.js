const Libpannellum = (function (window, document, undefined) {
  "use strict";
  ...
  function Renderer(container) {
    ...
    this.init = function (_image, _imageType, _dynamic, haov, vaov, voffset, callback, params) {
      ...
      if (imageType == "equirectangular") {
        ...
        if (Math.max(image.width / 2, image.height) > maxWidth) {
          console.log(
            "Error: The image is too big; it's " + image.width + "px wide, " + "but this device's maximum supported size is " + maxWidth * 2 + "px."
          );
          throw {
        ...
        if (cubeImgWidth > gl.getParameter(gl.MAX_CUBE_MAP_TEXTURE_SIZE)) {
          console.log(
            "Error: The image is too big; it's " + cubeImgWidth + "px wide, " + "but this device's maximum supported size is " + maxWidth + "px."
          );
          throw {
            type: "webgl size error",
            width: cubeImgWidth,
    ...
    this.render = function (pitch, yaw, hfov, params) {
      ...
      if (pose !== undefined) {
        ...
        var orig_pitch = pitch,
          ...
          x =
            ...
            Math.cos(pitch) * (Math.cos(horizonPitch) * Math.cos(yaw) + Math.sin(horizonRoll) * Math.sin(horizonPitch) * Math.sin(yaw)),
          ...
          z =
            ...
            Math.cos(pitch) * (-Math.cos(yaw) * Math.sin(horizonPitch) + Math.cos(horizonPitch) * Math.sin(horizonRoll) * Math.sin(yaw));
        ...
        var v = [
            Math.cos(orig_pitch) *
              (Math.sin(horizonRoll) * Math.sin(horizonPitch) * Math.cos(orig_yaw) - Math.cos(horizonPitch) * Math.sin(orig_yaw)),
            Math.cos(orig_pitch) * Math.cos(horizonRoll) * Math.cos(orig_yaw),
            Math.cos(orig_pitch) *
              (Math.cos(horizonPitch) * Math.sin(horizonRoll) * Math.cos(orig_yaw) + Math.sin(orig_yaw) * Math.sin(horizonPitch)),
          ],
          w = [-Math.cos(pitch) * Math.sin(yaw), Math.cos(pitch) * Math.cos(yaw)];
        var roll_adj = Math.acos(
          Math.max(
            Math.min((v[0] * w[0] + v[1] * w[1]) / (Math.sqrt(v[0] * v[0] + v[1] * v[1] + v[2] * v[2]) * Math.sqrt(w[0] * w[0] + w[1] * w[1])), 1),
            -1
          )
        );
      ...
      if (params.returnImage !== undefined) {
        if (window.createImageBitmap && params.returnImage == "ImageBitmap") {
          ...
          if (params.returnImage.toString().indexOf("image/") == 0) return canvas.toDataURL(params.returnImage, 0.75);
          else if (params.returnImage.toString().indexOf("screenshot") == 0) return canvas.toDataURL("image/jpeg", 1.0);
          else return canvas.toDataURL("image/jpeg", 0.75); // Old default
        }
      }
    };
    ...
    function multiresDraw(clear) {
      if (!program.drawInProgress) {
        ...
        // Draw tiles
        for (var i = 0; i < program.currentNodes.length; i++) {
          if (program.currentNodes[i].textureLoaded > 1 && node_paths[program.currentNodes[i].path] != 0) {
            // 1 or undefined
    ...
    function makeMatrix4(m) {
      return new Float32Array([m[0], m[1], m[2], 0, m[3], m[4], m[5], 0, m[6], m[7], m[8], 0, 0, 0, 0, 1]);
    }
    ...
    if (window.Worker && window.createImageBitmap) {
      ...
      workerFuncBlob = new Blob(["(" + workerFunc.toString() + ")()"], {
    ...
    function shtFlm2pixel(flm, Ylm, lon) {
      ...
      for (var l = lmax; l >= 0; l--) {
        ...
        for (var m = 1; m <= l; m++) expand += (flm[++idx] * cosm[m] + flm[idx + cosidx - l - 1] * sinm[m]) * Ylm[idx - 1];
      }
    ...
    function shtDecodeImage(shtHash) {
      if (shtYlm.length < 1) {
        ...
        for (var i = 0; i < 32; i++) {
          shtYlm.push([]);
          for (var j = 0; j < ylmLen; j++) shtYlm[i].push(shtDecodeFloat(shtB83decode(shtYlmStr[i * ylmLen + j], 1), 41) * shtMaxYlm);
        }
      }
