...
const Pannellum = (function (window, document, undefined) {
  "use strict";
  ...
  function Viewer(container, initialConfig) {
    ...
    function onDocumentMouseDown(event) {
      ...
      if (config.hotSpotDebug) {
        ...
        console.log(
          "Pitch: " + coords[0] + ", Yaw: " + coords[1] + ", Center Pitch: " + config.pitch + ", Center Yaw: " + config.yaw + ", HFOV: " + config.hfov
        );
      }
    ...
    function onDocumentMouseMove(event) {
      if (draggingHotSpot) {
        ...
        var yaw =
          ((((Math.atan((onPointerDownPointerX / canvasWidth) * 2 - 1) - Math.atan((pos.x / canvasWidth) * 2 - 1)) * 180) / Math.PI) * config.hfov) /
            90 +
          onPointerDownYaw;
        ...
        var pitch =
          ((((Math.atan((pos.y / canvasHeight) * 2 - 1) - Math.atan((onPointerDownPointerY / canvasHeight) * 2 - 1)) * 180) / Math.PI) * vfov) / 90 +
          onPointerDownPitch;
    ...
    function createHotSpot(hs) {
      ...
      if (hs.rotateX || hs.rotateZ) {
        hotspotMaker.style = `--transform: translateZ(9999px) rotateX(${hs.rotateX + "deg"}) rotateZ(${hs.rotateZ + "deg"})`;
      } else {
      ...
      if (hs.createTooltipFunc) {
        ...
        if (config.cssMaker == "custom-maker") {
          span.style.marginLeft = -(span.scrollWidth - hotspotMaker.offsetWidth) / 2 + "px";
        } else {
          span.style.marginLeft = -(span.scrollWidth - div.offsetWidth) / 2 + "px";
        }
        span.style.marginTop = -span.scrollHeight - 17 + "px";
      }
          ...
          switch (key) {
            ...
            case "showFullscreenCtrl":
              if (
                ...
                ("fullscreen" in document || "mozFullScreen" in document || "webkitIsFullScreen" in document || "msFullscreenElement" in document)
              ) {
    ...
    function constrainHfov(hfov) {
      ...
      if (config.avoidShowingBackground && renderer && !isNaN(config.maxPitch - config.minPitch)) {
        ...
        newHfov = Math.min(
          newHfov,
          (Math.atan((Math.tan(((config.maxPitch - config.minPitch) / 360) * Math.PI) / canvas.height) * canvas.width) * 360) / Math.PI
        );
      }
      return newHfov;
    }
